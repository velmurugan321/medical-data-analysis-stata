...
const modules = [
  ...
  { title: 'Variable View', icon: 'list-outline', text: 'Types, labels, missing and unique values' },
...
function summarize(rows, metadata = {}) {
  ...
  return columns.map((name) => {
    ...
    return { name, type: meta.type || (isNumeric ? 'Numeric' : 'Categorical/Text'), label: meta.label || '', stataFormat: meta.stataFormat || '', valueLabel: meta.valueLabel || '', valueLabels: meta.valueLabels || {}, missing: values.length - nonMissing.length, unique: new Set(nonMissing).size, mean, sd: variance === null ? null : Math.sqrt(variance), median };
  });
}
...
export default function App() {
  ...
  async function uploadStata(asset) {
    ...
    if (!response.ok) {
      ...
      throw new Error(message || 'Stata server could not read the dataset.');
    }
  ...
  async function pickData() {
    try {
      const result = await DocumentPicker.getDocumentAsync({ type: ['text/csv', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel', 'application/octet-stream'], copyToCacheDirectory: true });
      ...
      if (lower.endsWith('.csv')) {
        ...
        metadata = Object.fromEntries((response.columns || []).map((v) => [v.name, v]));
      } else {
      ...
      Alert.alert('Upload error', error.message || 'The dataset could not be read.');
    } finally {
      setLoading(false);
    }
  }

  const summary = useMemo(() => summarize(rows, stataMetadata), [rows, stataMetadata]);
  const numericVars = summary.filter((v) => v.mean !== null);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.status}><Ionicons name="shield-checkmark-outline" size={22} color="#0f766e" /><View style={{ flex: 1 }}><Text style={styles.statusTitle}>Stata-aware validation</Text><Text style={styles.statusText}>.dta metadata, variable labels and value labels are read by the analysis backend. Statistical methods will be validated against R/Stata before production release.</Text></View></View>
      </ScrollView>
    </SafeAreaView>
  );
}
