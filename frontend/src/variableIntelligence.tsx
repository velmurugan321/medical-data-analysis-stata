...
type Sheet = { sheet:string; status:string; rows?:number; columns?:number; variables?:Profile[] }
type Workbook = { filename:string; sheet_count:number; sheets:Sheet[] }
type Props = { files:File[]; onComplete:(contract:any[])=>void }

const API='https://medical-data-analysis-api.onrender.com'

export function VariableIntelligence({files,onComplete}:Props){
 const [data,setData]=useState<{files:Workbook[]}|null>(null)
 const [busy,setBusy]=useState(false)
 const [review,setReview]=useState<Record<string,string>>({})
 const [error,setError]=useState('')
 const [open,setOpen]=useState(false)
 const profiles:Profile[]=useMemo(()=>data?.files.flatMap((f:Workbook)=>f.sheets.flatMap((s:Sheet)=>(s.variables||[]).map((v:Profile)=>({...v,file:f.filename,sheet:s.sheet}))))||[],[data])
 ...
 async function inspect(){
  ...
  try{const fd=new FormData();files.forEach(f=>fd.append('files',f));const r=await fetch(`${API}/api/v1/data/inspect-workbooks`,{method:'POST',body:fd});if(!r.ok)throw Error(await r.text());const d=await r.json();setData(d);const initial:Record<string,string>={};d.files.forEach((f:Workbook)=>f.sheets.forEach((s:Sheet)=>(s.variables||[]).forEach((v:Profile)=>{const k=`${f.filename}|${s.sheet}|${v.name}`;if(!needs(v))initial[k]=suggested(v)})));setReview(initial);setOpen(true)}catch(e){setError(e instanceof Error?e.message:'Inspection failed')}finally{setBusy(false)}
 }
 function proceed(){const missing=profiles.filter((v:Profile)=>!review[`${v.file}|${v.sheet}|${v.name}`]);if(missing.length){setError(`${missing.length} variable(s) still need confirmation.`);return}const contract=profiles.map((v:Profile)=>({...v,confirmed_type:review[`${v.file}|${v.sheet}|${v.name}`],coding:{}}));onComplete(contract)}
